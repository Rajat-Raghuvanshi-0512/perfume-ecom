...
export function CartDrawer({
  ...
  const handleProceedToAddress = () => {
    if (!session?.user) {
      ...
      // Optionally close the drawer so they can log in via the Navbar button
      onClose();
      return;
    }
    fetchAddresses();
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      toast.add({
        title: "Error",
        description: "Error connecting to payment gateway",
        type: "error",
      });
    } finally {
        ...
        <div className="w-screen max-w-md bg-[#0F0F12] border-l border-white/10 text-[#F5F5F0] flex flex-col justify-between shadow-2xl">
            ...
            <div className="flex items-center gap-3">
              {checkoutStep === "ADDRESS" && (
                <button
                  ...
                  className="p-2 mr-2 text-[#888880] hover:text-[#D4AF37] border border-white/10 rounded-full transition-colors flex items-center justify-center"
              ...
              <div>
                <h2 className="text-sm sm:text-base font-serif tracking-[0.2em] uppercase font-light text-[#F5F5F0]">
                  {checkoutStep === "CART" ? "Your Velvet Coffer" : "Delivery Address"}
                </h2>
          ...
          {checkoutStep === "CART" ? (
            ...
            <>
              ...
              <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-4">
                {loadingAddresses ? (
                  ...
                  <div className="space-y-3">
                    ...
                    {addresses.length === 0 ? (
                      <div className="py-8 text-center text-xs text-[#A0A098]">
                        <p>You don't have any saved addresses.</p>
                        <button
                          onClick={() => setShowAddAddressModal(true)}
                          className="mt-3 px-4 py-2 border border-[#D4AF37] text-[#D4AF37] hover:bg-[#D4AF37]/10 transition-colors uppercase tracking-widest"
                        >
                          Add an Address
                        </button>
                      </div>
              ...
              {/* Bottom Checkout Section (Address Mode) */}
              <div className="p-4 sm:p-6 bg-[#141418] border-t border-white/10 space-y-3">
                <div className="flex justify-between text-sm text-[#F5F5F0] font-serif font-bold pb-2">
                  <span>Total Due</span>
                  <span className="text-[#D4AF37]">Rs. {(subtotal + shippingFee).toLocaleString("en-IN")}</span>
                </div>
                <button
                  onClick={handleCheckout}
                  disabled={isCheckingOut || !selectedAddressId || addresses.length === 0}
